...
- **v2.3.5** 专注模式复合块高亮修复 + Rust 命令单测补全 + 版本号同步：①issue #56 修复专注模式下点击列表/表格当前块不高亮——`editor-modes.ts` 的装饰原先取光标所在「最内层块」（`findParentNodeClosestToPos(n => n.isBlock)`），列表/表格内命中内部 paragraph，而 CSS 只高亮 `.ProseMirror` 直接子节点，导致外层列表/表格被整体弱化到 0.35 点不亮；改为取光标所在「文档顶层块」（`$head.node(1)`，即 `.ProseMirror` 直接子节点），装饰粒度与 CSS 高亮粒度一致，列表→`bullet_list`/`ordered_list`、表格→`table` 均正确点亮，新增 `tests/unit/editor-modes.test.ts` 5 个用例验证各块类型；②issue #47 为 `search.rs` 的 `search_in_workspace` 补 9 个单测（空查询/工作区不存在/大小写切换/非法正则/跨文件行号与路径/隐藏目录跳过/非 UTF-8 静默跳过/UTF-8 列号计数/超大文件跳过），复用 `mod.rs` 既有临时目录模式；③issue #48 为 `pandoc.rs` 补单测并做「参数拼装与执行分离」重构（`build_pandoc_command`/`run_pandoc`），用注入假脚本覆盖 `--resource-path` 追加/非目录忽略/pandoc 缺失/非零退出码/成功 5 个分支，无需 CI 安装 pandoc；④CI `test` job 增加 Rust `cargo test` 步骤；⑤同步 Cargo.toml/Cargo.lock/tauri.conf.json/package.json 版本号至 2.3.5（此前 Cargo.toml 滞后在 2.2.0）。前端 376 单测 + `tsc --noEmit` 全绿。详见 `docs/v2.3.5 设计文档.md`
- **v2.3.4** 打开瞬间抖动根治 + 切 tab 大纲定位修复：v2.3.3 后用户实测仍有两个问题——①打开文件瞬间抖动一下且文件越大越久；②切 tab 后大纲高亮停在顶部，需手动滚动才恢复。问题①根因是打开路径上三个"渐进改变高度"环节都在首帧后发生、滚动锚定逐次补偿：代码块懒挂载前 `cmHost` 为空 div（高度≈0，挂载后撑开数百 px，首屏短代码块越多跳得越久，主因）、Mermaid 未缓存首渲染从粗估高度跳到实测并强制收缩、滚动恢复被未撑开的 scrollHeight 钳制且 rAF 仅重试一次。修复：①代码块挂载前用与 CodeMirror 同字体/行高/padding/max-height 的 `<pre>` 纯文本占位（挂载前后高度差≈0，附带挂载前即可见代码内容），视口外内容变更同步占位文本；②Mermaid 首渲染 min-height 取 max(占位, 实测) 只增不减，注入零跳变；③滚动恢复逐帧重试到 30 帧上限。问题②为 v2.3.3 重构引入：切 tab 重灌文档后选区被钳到文档头，大纲重算按选区推导高亮跳回顶部且此后无 scroll 事件触发采样。修复：重算完成后按当前 scrollTop 采样定位（阅读位置语义），防抖窗口内标记 stale 跳过采样与选区推导（避免旧文档标题集产出错误高亮），插件创建后追加 rAF 初始采样兜底。实测（1.8 万行/398KB 压测文档，缓存冷启动）：打开全程编辑器零布局位移、scrollTop 零漂移，scrollHeight 初始与最终偏差 0.67%；滚至中部→切走→切回，scrollTop 与大纲高亮（第 109 项）均自动恢复，无需滚动。详见 `docs/v2.3.4 设计文档.md`
- **v2.3.3** 大文档窗口抖动 + 引用块滚动掉帧根治：用户实测 v2.3.2 反馈打开大文件后窗口一直抖动、引用块区域滚动仍明显掉帧（v2.1.0 丝滑）。两个独立根因：①视口上方的图表后台预渲染变高触发滚动锚定反复补偿 scrollTop（抖动），且压测文档 60 张图表仅 8 种源码，重复图表每张仍 ~150ms 全量渲染形成 ~9s 预渲染风暴与滚轮撞车；②v2.2.0 大纲自动跟随的 `posAtCoords` 采样在万行文档上线性扫描文档级子节点 rect，引用块（深层嵌套）区域单次 55-67ms（PerformanceObserver 实测 6 个长任务、周期恰为 120ms 节流 + 60ms 采样耗时）。修复：①`mermaid-view.ts` 按源码 LRU 缓存 SVG + 实测高度——重复图表命中缓存仅 ~2ms DOM 注入且创建即预留精确高度（占位→渲染高度跳变为 0），空闲队列跳过整体位于视口上方的图表（上方布局不再变化，锚定补偿消失）；②`outline-tracker.ts` 滚动采样重构——批量缓存标题元素在滚动坐标系中的位置，采样退化为 scrollTop 与缓存数组二分比较（微秒级）+ 120ms 节流，彻底移除 posAtCoords，缓存随文档变更防抖重建并在滚动总高/宽度变化时自动重建。实测（1.8 万行/398KB/60 图表/371 引用块压测文档）：预渲染 60 张 ~9s → ≤5s（8 次真实渲染 + 52 次缓存命中），打开全程 scrollHeight 稳定（CLS 0.00、静置 scrollTop 零漂移）；引用块密集区滚动 94fps/15 帧 >34ms/6 个 55-67ms 长任务 → 105fps/2 帧 >34ms/0 长任务（p95 35→25ms）；向上滚动 119fps/0 帧 >34ms。全套单元/组件测试 369 passed。详见 `docs/v2.3.3 设计文档.md`
