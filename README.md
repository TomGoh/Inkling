...
- **v2.3.8** 批量修复 #59-#67：保存前磁盘基线比对（#59，外部已改弹确认防静默覆盖）+ 草稿贴图 Data URL 内联（#60）+ 多窗口主题/偏好实时同步（#61）+ 全局搜索按第 N 处匹配定位（#62）+ Pandoc 临时文件防并发覆盖（#63）+ asset 协议权限收敛（#64）+ 嵌套列表删块精准化（#65）+ PNG 导出离屏样式对齐（#67）+ 开发文档完善（#66）
- **v2.3.7** 外部文件变动冲突对话框（口头反馈：另存副本/差异对比/重载/继续编辑四选项，消除静默覆盖风险）+ CI 发版前置校验（版本号一致性 + 文档同步，仅 tag 时强制）
