...
- **v2.3.6** 文档与工程化收尾：#49 拆分 workspace store 为 4 个 Zustand slice；#50 拆分 Sidebar / Editor / App 三个巨型组件（行为不变，前端 376 单测 + `tsc` 全绿）；#51 版本历史迁移到独立 `CHANGELOG.md`；#52 新增 `ARCHITECTURE.md` 架构总览
- **v2.3.5** 专注模式复合块高亮修复（#56）+ Rust 命令单测补全（#47/#48）+ 版本号同步
- **v2.3.4** 打开瞬间抖动根治（代码块纯文本占位 + Mermaid 高度预留 + 滚动恢复逐帧重试）+ 切 tab 大纲定位修复
- **v2.3.3** 大文档窗口抖动 + 引用块滚动掉帧根治（Mermaid LRU 缓存 + 大纲 scrollTop 二分采样）
- **v2.3.2** 万行多图文档滚动掉帧修复（Mermaid 空闲预渲染队列）
- **v2.3.1** 万行多图文档打开卡顿修复（Mermaid 视口懒渲染）
