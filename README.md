...
- **v3.2.0** 性能回归防线基建（#216，PR #231，17 个提交 / 50 文件 +6324−2）——**本版本不含产品代码改动**（`src/`、`src-tauri/` 零改动），全部为工程质量投入：新增 `pnpm run benchmark`（6 场景 × 4 档位 × 真实负载 fixture，median 与 p95 双行参与判定），建立三层判定——①**噪声门槛 3σ**（基线保留最近 8 次运行，按实测跨运行散布判定：`inputSyncMs` 3σ=1.25ms、`ttiMs` 3σ=241ms、而 vsync 量化的 `frameMs.p95` 仅 0.39ms，用固定百分比必然误判）、②**判定分层**（主指标可单独判 FAIL，派生指标需主指标佐证，小量级指标另有实测绝对地板）、③**两阶段复现**（首轮超阈值只复测该场景，仍超判 FAIL、回落判 WARN，退出码 0/1/2）；基线身份 = 测量配置（`<profile>/<mode>/r<rounds>/` 路径隔离，headless 日常回归与 uncapped 120fps 定向验证互不覆盖），滚动帧预算绝对目标仅在定向模式产出。新增 CI 工作流 `benchmark.yml`（PR 触发、不阻断合并）。单测 760→840 例，每个修复带变异验证。详见 `docs/v3.2.0 设计文档.md`
