...
- **v2.7.1** 模式切换滚动恢复单一写者与光标可见性保障（#136：useCursorStateRestore 翻转让位 + 源码进入 settle 循环收敛 + 退出恢复写回 tab 记忆 + coordsAtPos 最小滚动校正保证光标可见；524 单测 / 157 E2E 全绿）
...
- **v2.6.2** 评审遗留深度加固与数据可靠性（Rust 原子落盘与 Nonce 临时文件唯一性 + IPC 32KB 分块 Base64 传输 + 外部删除快照恢复面板 + 毫秒级 mtime 精度统一与 Fast-Path + 自定义 CSS 持久化 + 全量测试 TS 严格类型健全 + 统一原生对话框）
- **v2.6.1** 架构加固与缺陷修复（#91 保存死锁与冲突取消覆盖修复 + #86 CSS 16进制转义防御 + #111 CSP策略收紧 + #100 自动保存指数退避与非阻塞模式 + #89 Rust fsync 物理落盘 + 真实测试套件全面重构）
- **v2.6.0** 32 项 GitHub Issues 全面攻坚（原子写盘防截断 + 异步非阻塞搜索 + 保存快照隔离 + 导出强同步 flush + XSS/CSS注入防御 + Myers/LCS空间优化 + 统一模块与全量测试）
