...
describe("source-mode-navigation", () => {
  ...
  it("getSourceModeScroll 获取与清理正常工作", () => {
    registerSourceModeScroll("/test.md", {
      ...
      getScrollAndCursor: () => ({ scrollTop: 150, cursor: 42, scrollHeight: 300, anchorOffset: 120, cursorVisible: true }),
    });

    const scroll = getSourceModeScroll("/test.md");
    expect(scroll).toEqual({ scrollTop: 150, cursor: 42, scrollHeight: 300, anchorOffset: 120, cursorVisible: true });
