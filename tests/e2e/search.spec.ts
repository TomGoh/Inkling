...
test.describe("查找替换", () => {
  ...
  test("输入查找词显示匹配计数", async ({ page }) => {
    ...
    // 等待防抖 + 搜索
    await expect(page.locator(".search-count")).toBeVisible({ timeout: 5000 });
  ...
  test("Ctrl+R 打开替换面板（含替换框）", async ({ page }) => {
    ...
    await expect(page.locator(".search-panel")).toBeVisible();
    // 替换框应显示
    await expect(page.locator(".search-input").nth(1)).toBeVisible();
  });
  ...
  test("查找导航：下一个/上一个按钮", async ({ page }) => {
    ...
    await page.getByText("readme.md").click();
    await page.keyboard.press("Control+f");
    await page.locator(".search-input").first().fill("o"); // "o" 在 readme 中多次出现
    await page.waitForTimeout(300); // 等防抖
