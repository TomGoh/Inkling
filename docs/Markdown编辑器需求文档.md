...
| 3.3 文件与工作区 | 外部修改冲突对话框 | ✅ | v2.3.7 | issue #58：本地 dirty 且磁盘被外部修改时弹冲突对话框（保留本地另存 `.backup.md` 副本 / 行级差异对比 / 丢弃修改重载 / 继续编辑），替代原 confirm 二选一，消除「取消后保存静默覆盖外部修改」的数据丢失风险 |
