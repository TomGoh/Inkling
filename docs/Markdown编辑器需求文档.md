...
- **v2.5.9** 根路径边界与跨平台路径兼容加固（闭环全量 Issues 与技术债）：①修复 Windows 根盘符及 POSIX 根目录下文件取父目录时被错误截断为空字符串的问题（`parentDir` / `dirNameOf` 处理 `/file.md`、`C:\file.md` 时精准保留根目录 `/` 与 `C:\`），保障资源权限放行与树结构局部刷新稳定性；②补充根目录边界单测覆盖；③全量闭环 GitHub 32 项 Issues。详见 `docs/v2.5.9 设计文档.md`
